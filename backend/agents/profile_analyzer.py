...
class ProfileAnalyzerAgent:
    ...
    def __init__(self, model_name: Optional[str] = None, temperature: Optional[float] = None):
        self.llm = ChatOpenAI(
            ...
            temperature=temperature or (config.DEFAULT_TEMPERATURE + 0.1),  # Slightly higher for analysis
    ...
    async def analyze_profile(self, profile_data: Dict[str, Any], target_role: Optional[str] = None, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"🔍 [REQ:{req_id}] Profile Analyzer - Starting analysis for target role: {target_role or 'General'}")
        ...
        try:
            ...
            logger.info(f"🤖 [REQ:{req_id}] Sending analysis request to {config.OPENAI_MODEL}...")
            llm_start = time.time()
            response = await self.llm.ainvoke(messages)
            llm_time = time.time() - llm_start

            logger.info(f"🤖 [REQ:{req_id}] Analysis response received in {llm_time:.2f}s")
            ...
            try:
                ...
                logger.info(f"✅ [REQ:{req_id}] Profile analysis completed in {total_time:.2f}s")
                logger.info(f"📊 [REQ:{req_id}] Analysis score: {validated_results.get('overall_score', 'N/A')}/100")
            ...
            total_time = time.time() - start_time
            logger.error(f"💥 [REQ:{req_id}] Profile analysis failed after {total_time:.2f}s: {str(e)}")
