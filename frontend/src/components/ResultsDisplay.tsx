  ...
  if (!results.success) {
    return (
      <div className="max-w-4xl mx-auto p-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
        ...
        <p className="text-red-600 dark:text-red-400">{results.error || 'Unknown error occurred'}</p>
        <p className="text-sm text-red-500 dark:text-red-400 mt-2">Status: {results.status}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 mb-6">
        ...
        <p className="text-green-600 dark:text-green-400">{results.status}</p>
      </div>
...
function OverviewTab({ results }: { results: OptimizationResults }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-gray-600 dark:text-gray-300 font-medium">Content Ideas:</span>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-gray-600 dark:text-gray-300 font-medium">Sample Posts:</span>
      ...
      {/* Key Improvements */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm md:col-span-2 lg:col-span-3">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">
          🎯 Priority Actions
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-medium text-gray-700 dark:text-gray-300 mb-2">Top Improvements:</h4>
          ...
          <div>
            <h4 className="font-medium text-gray-700 dark:text-gray-300 mb-2">Recommended Actions:</h4>
            <ul className="space-y-1">
    ...
    <div className="space-y-6">
      ...
      {analysis.industry_insights && (
        <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <p className="text-blue-700 dark:text-blue-400">{analysis.industry_insights}</p>
        </div>
      )}
      ...
      {activeContentTab === 'ideas' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(content.content_ideas || []).map((idea: any, index: number) => (
            <div key={index} className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm">
              <div className="flex justify-between items-start mb-3">
                <h4 className="font-semibold text-gray-800 dark:text-white">{idea.topic}</h4>
                <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-xs rounded text-gray-600 dark:text-gray-300">
                  {idea.type}
                </span>
              </div>
              <p className="text-gray-600 dark:text-gray-400 text-sm mb-3">{idea.objective}</p>
              <p className="text-gray-700 dark:text-gray-300 mb-3">{idea.content}</p>
              <div className="space-y-2">
                <div>
                  <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Target:</span>
                  <span className="text-xs text-gray-600 dark:text-gray-400 ml-1">{idea.target_audience}</span>
                </div>
                <div>
                  <span className="text-xs font-medium text-gray-500 dark:text-gray-400">CTA:</span>
                  <span className="text-xs text-gray-600 dark:text-gray-400 ml-1">{idea.call_to_action}</span>
                </div>
      ...
      {activeContentTab === 'posts' && (
        <div className="space-y-6">
          {(content.sample_posts || []).map((post: any, index: number) => (
            <div key={index} className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm">
              <h4 className="font-semibold text-gray-800 dark:text-white mb-3">{post.title}</h4>
      ...
      {recommendations.headline && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <h4 className="font-medium text-red-600 dark:text-red-400 mb-2">Current:</h4>
              <p className="text-gray-700 dark:text-gray-300 bg-red-50 dark:bg-red-900/20 p-3 rounded">
                {recommendations.headline.current || 'Not provided'}
              </p>
            </div>
            <div>
              <h4 className="font-medium text-green-600 dark:text-green-400 mb-2">Suggested:</h4>
            ...
            <div>
              <h4 className="font-medium text-blue-600 dark:text-blue-400 mb-2">Why this works:</h4>
      ...
      {recommendations.summary && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border dark:border-gray-700 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <h4 className="font-medium text-red-600 dark:text-red-400 mb-2">Current:</h4>
              <p className="text-gray-700 dark:text-gray-300 bg-red-50 dark:bg-red-900/20 p-3 rounded text-sm">
                {recommendations.summary.current || 'Not provided'}
              </p>
            </div>
            <div>
              <h4 className="font-medium text-green-600 dark:text-green-400 mb-2">Suggested:</h4>
              <p className="text-gray-700 dark:text-gray-300 bg-green-50 dark:bg-green-900/20 p-3 rounded text-sm">
                {recommendations.summary.suggested}
              </p>
            </div>
            <div>
              <h4 className="font-medium text-blue-600 dark:text-blue-400 mb-2">Why this works:</h4>
              <p className="text-gray-700 dark:text-gray-300 bg-blue-50 dark:bg-blue-900/20 p-3 rounded text-sm">
